...
#property description "Super-Bot: Multi-stage trading EA from $1 to $200+"
...
//--- Global variables
int handleEMAFast, handleEMASlow, handleRSI, handleBB;
...
int OnInit()
{
   ...
   handleEMAFast = iMA(Symbol(), PERIOD_M15, InpEMAFast, 0, MODE_EMA, PRICE_CLOSE);
   handleEMASlow = iMA(Symbol(), PERIOD_M15, InpEMASlow, 0, MODE_EMA, PRICE_CLOSE);
   handleRSI = iRSI(Symbol(), PERIOD_M15, InpRSIPeriod, PRICE_CLOSE);
   handleBB = iBands(Symbol(), PERIOD_M15, InpBBPeriod, 0, InpBBDeviation, PRICE_CLOSE);
   ...
   isInitialized = true;
   Print("SuperBot EA initialized successfully. Current stage: ", EnumToString(currentStage));
...
void OpenPosition(ENUM_ORDER_TYPE orderType, string comment)
{
   ...
   if(orderType == ORDER_TYPE_BUY)
   {
      ...
      sl = price - InpStopLoss * SymbolInfoDouble(Symbol(), SYMBOL_POINT) * 10;
      tp = price + InpTakeProfit * SymbolInfoDouble(Symbol(), SYMBOL_POINT) * 10;
   }
   else
   {
...
void ApplyTrailingStop()
{
   ...
   if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
   {
      ...
      newSL = currentPrice - InpTrailingDistance * SymbolInfoDouble(Symbol(), SYMBOL_POINT) * 10;
